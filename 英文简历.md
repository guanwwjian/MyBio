...
- Collect and update client push id based on pingback receipt log
...
- Realization principle: Undertake the CTR estimation score calculated by the CTR prediction model prediction process, maintain a priority queue for each user, and take out the leader article of each user when receiving the push instruction, aggregate by article, and submit Push
...
- Advantage:
    ...
    - Simple maintenance: When new fields appear in the log, modify the hive view without creating a new Hive table will add new fields support.
- Architecture:
